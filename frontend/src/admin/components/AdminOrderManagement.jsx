        ...
        {focus && (
          <div className="space-y-6">
            ...
            {focus.items && focus.items.length > 0 && (
              <div className="p-4 rounded-2xl bg-gradient-to-br from-emerald-50 to-green-50 border border-emerald-200 shadow-sm">
                <div className="flex items-center gap-2 mb-4">
                  <div className="w-8 h-8 rounded-full bg-emerald-100 flex items-center justify-center text-emerald-700">
                    <Wallet className="w-5 h-5" />
                  </div>
                  <h3 className="text-lg font-bold text-gray-900">Seller Payout Breakdown</h3>
                </div>
                ...
                <div className="space-y-4">
                  {(() => {
                    ...
                    (focus.items || []).forEach(item => {
                      // Determine if this is a 3rd party seller (Designer/Reseller)
                      const sellerTypeLower = (item.sellerType || '').toLowerCase();
                      const isThirdParty = item.sellerId && (sellerTypeLower === 'designer' || sellerTypeLower === 'reseller');

                      const isAdminProduct = !isThirdParty;

                      const effectiveSellerId = isAdminProduct ? 'admin' : item.sellerId;
                      const effectiveSellerType = isAdminProduct ? 'Admin' : (item.sellerType || 'Seller');

                      // Try to get name from Item -> Cache -> Fallback
                      let effectiveSellerName = isAdminProduct ? 'Admin (Platform)' : (item.sellerName || 'Seller');

                      // If we have cached details, use the fresh name
                      if (!isAdminProduct && sellerDetails[effectiveSellerId]?.fullName) {
                        effectiveSellerName = sellerDetails[effectiveSellerId].fullName;
                      }

                      const key = `${effectiveSellerId}_${effectiveSellerType}`;
                      if (!sellerGroups[key]) {
                        sellerGroups[key] = {
                      ...
                      sellerGroups[key].subtotal += item.lineTotal || 0;
                    });

                    return Object.values(sellerGroups).map((group, idx) => {
                      const platformFee = group.isAdmin ? 0 : group.subtotal * 0.10; // No fee for admin products
                      const payableToSeller = group.subtotal - platformFee;
                      const sellerInfo = group.isAdmin ? null : sellerDetails[group.sellerId];
                      ...
                      return (
                        <div key={idx} className="bg-white p-4 rounded-xl border border-emerald-100 shadow-sm">
                          ...
                          <div className="flex items-start justify-between mb-3 pb-3 border-b border-gray-100">
                            <div>
                              <p className="font-bold text-gray-900">{group.sellerName}</p>
                              <p className="text-xs text-gray-500 capitalize">{group.sellerType}</p>
                            </div>
                            <span className={`px-2 py-1 text-xs font-bold rounded-full ${group.isAdmin ? 'bg-purple-100 text-purple-700' : 'bg-emerald-100 text-emerald-700'}`}>
                            ...
                            <div className="mb-3 p-2 bg-gray-50 rounded-lg text-xs space-y-1">
                              {sellerInfo.email && (
                                <p className="flex items-center gap-2 text-gray-600">
                                  <Mail className="w-3 h-3" /> {sellerInfo.email}
                                </p>
                              )}
                              {sellerInfo.phone && (
                                <p className="flex items-center gap-2 text-gray-600">
                                  <Phone className="w-3 h-3" /> {sellerInfo.phone}
                                </p>
                              )}
                              {sellerInfo.location && (
                                <p className="flex items-center gap-2 text-gray-600">
                                  <MapPin className="w-3 h-3" /> {sellerInfo.location}
                                </p>
                              )}
                              {sellerInfo.address && (
                                <p className="flex items-center gap-2 text-gray-600">
                                  <Home className="w-3 h-3" /> {sellerInfo.address}
                                </p>
                              )}
                            </div>
                          )}
                          ...
                          <div className="space-y-2 text-sm">
                            <div className="flex justify-between">
                              ...
                              <span className="font-medium">{currency(group.subtotal)}</span>
                            </div>
                            ...
                            <div className="flex justify-between pt-2 border-t border-dashed border-gray-200">
                              <span className="font-bold text-gray-900">Payable to Seller</span>
                              <span className="font-bold text-emerald-700">{currency(payableToSeller)}</span>
                            </div>
                          </div>

                          {/* Payout Method & Details */}
                          {sellerInfo && sellerInfo.paymentMethod && (
                            <div className="mt-3 p-3 bg-blue-50 rounded-xl border border-blue-100">
                              <div className="flex justify-between items-start mb-2">
                                <p className="text-xs font-bold text-blue-900 flex items-center gap-1">
                                  <Building className="w-3 h-3" /> Payout Account
                                </p>
                                <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase flex items-center gap-1 ${sellerInfo.payoutStatus === 'verified'
                                  ? 'bg-green-100 text-green-700'
                                  : sellerInfo.payoutStatus === 'rejected'
                                    ? 'bg-red-100 text-red-700'
                                    : 'bg-amber-100 text-amber-700'
                                  }`}>
                                  {sellerInfo.payoutStatus === 'verified' ? <ShieldCheck className="w-2.5 h-2.5" /> : <ShieldAlert className="w-2.5 h-2.5" />}
                                  {sellerInfo.payoutStatus || 'Pending'}
                                </span>
                              </div>
                              <p className="text-xs text-gray-700">
                                <span className="font-semibold text-gray-900">{sellerInfo.paymentMethod}</span>
                                {(sellerInfo.paymentMethod === 'JazzCash' || sellerInfo.paymentMethod === 'EasyPaisa') && sellerInfo.paymentDetails?.accountNumber && (
                                  <span className="ml-1">
                                    • {sellerInfo.paymentDetails.accountName || 'N/A'}
                                    <span className="font-mono ml-1 text-gray-500">
                                      (...{sellerInfo.paymentDetails.accountNumber.slice(-4)})
                                    </span>
                                  </span>
                                )}
                                {sellerInfo.paymentMethod === 'BankAccount' && sellerInfo.paymentDetails?.bankAccountNumber && (
                                  <span className="ml-1">
                                    • {sellerInfo.paymentDetails.bankName || 'Bank'}
                                    <span className="font-mono ml-1 text-gray-500">
                                      (...{sellerInfo.paymentDetails.bankAccountNumber.slice(-4)})
                                    </span>
                                  </span>
                                )}
                              </p>
                          ...
                          {(!sellerInfo || !sellerInfo.paymentMethod) && (
                            <div className="mt-3 p-2 bg-amber-50 rounded-lg border border-amber-200">
                              <p className="text-xs text-amber-700 font-medium">
                                ⚠ No payout details on file for this seller.
                              </p>
                            </div>
                          )}
                        </div>
